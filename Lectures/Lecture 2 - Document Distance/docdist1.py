...
# then the angle between them is de  ed as:
...
#O(w**2) where w is the number of words
def get_words_from_line_list(L):
    ...
    for line in L:
        ...
        word_list = word_list + words_in_line   #Adding the list creates a NEW list, so every element must be added
    return word_list


#O(n) since you must iterate through every character in the list
...
#O(w**2) since it needs to look at n words n times (looks at them once in word_list and again when they are added to L).
